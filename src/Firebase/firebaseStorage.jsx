...
export const fetchZoneFilesFromFolder = async (zone) => {
  try {
    ...
    const tif2023Files = await Promise.all(
      tifListResult.items
        .filter((item) => item.name.endsWith('.tif') && [item.name.includes('2023') || item.name.includes('new')])
        .map(async (item) => {
          const tif2023Url = await getDownloadURL(item);
          var fileName = '';
          if(item.name.includes('2023')){
            fileName = item.name.replace('.tif', '').replace('_2023', '');
          }
          else if(item.name.includes('new')){
            fileName = item.name.replace('.tif', '').replace('_new', ''); 
          }
...
const uploadTiffFile = async (zone, trackerName, file1, file2) => {
  try {
    // Create file names based on zone and tracker name
    const fileName1 = `Z${zone}_${trackerName}.tif`;
    const fileName2 = `Z${zone}_${trackerName}_new.tif`;

    // Create storage references
    const fileRef1 = ref(storage, `Zone${zone}/${fileName1}`);
    const fileRef2 = ref(storage, `Zone${zone}/${fileName2}`);
    ...
    console.error('Error uploading .tif files:', error);
    throw error;
  }
};